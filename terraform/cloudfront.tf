resource "aws_cloudfront_distribution" "s3_distribution" {
    origin {
    domain_name              = aws_s3_bucket.terraformbucket.bucket_regional_domain_name
    origin_id                = "S3-${aws_s3_bucket.terraformbucket.id}"
  ...
  default_cache_behavior {
    ...
    target_origin_id = "S3-${aws_s3_bucket.terraformbucket.id}"
